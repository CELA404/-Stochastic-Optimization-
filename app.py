...
# Imports time module to measure execution time
...
# ====================== DATA LOADING ======================
# ====================== DATA LOADING WITH AUTO DOWNLOAD ======================
st.sidebar.header("📁 Data Input")


@st.cache_data(show_spinner=True, ttl=3600)  # Cache for 1 hour
def load_ev_data():
    file_path = "SYNTHETIC_EV_DATA.csv"
    ...
    st.sidebar.info("📥 Downloading dataset for the first time... (120 MB - may take 30-60 seconds)")


    direct_url = "https://drive.google.com/file/d/1I6Q62MaMWCZl8Cc2g2PWfmNC5RZXzO3Y/view?usp=drive_link"
    # =================================================================

    try:
        with st.spinner("Downloading large dataset... Please wait"):
            response = requests.get(direct_url, stream=True, timeout=120)
            response.raise_for_status()

            # Αποθήκευση του αρχείου τοπικά (στο Streamlit Cloud)
            with open(file_path, "wb") as f:
                for chunk in response.iter_content(chunk_size=8192):
                    f.write(chunk)

            df = pd.read_csv(file_path)
            st.sidebar.success("✅ Dataset downloaded and loaded successfully!")
            return df

    except Exception as e:
        st.error(f"❌ Failed to download dataset.\nError: {str(e)}")
        st.error("Please try uploading the file manually.")
        # Fallback: File uploader
        uploaded_file = st.sidebar.file_uploader("Upload SYNTHETIC_EV_DATA.csv", type=["csv"])
...
st.caption("Fast Sensitivity Analysis • 30% simulation days • Ready for demonstration")  # Final caption at the bottom