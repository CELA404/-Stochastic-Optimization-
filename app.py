...
def load_ev_data():
    file_path = "C:\\Users\\ailis\\Downloads\\SYNTETIC_EV_DATA.csv"


    if os.path.exists(file_path):
        try:
            df = pd.read_csv(file_path)
            st.sidebar.success("✅ Dataset loaded from local storage")
            return df
        except Exception:
            pass

    # ==================== ΒΑΛΕ ΕΔΩ ΤΟ DIRECT LINK ΣΟΥ ====================
    direct_url = "https://drive.google.com/file/d/1I6Q62MaMWCZl8Cc2g2PWfmNC5RZXzO3Y/view?usp=drive_link"
    # =====================================================================

    st.sidebar.info("📥 Downloading dataset (120 MB)... This may take 30-90 seconds.")

    try:
        with st.spinner("Downloading large file from Google Drive..."):
            session = requests.Session()

            # Πρώτο request για να πάρουμε το confirmation token
        ...
        st.error("❌ Failed to download or read the dataset.")
        st.error(f"Error: {str(e)}")

        # Fallback: Manual upload
        uploaded_file = st.sidebar.file_uploader(
            "Upload SYNTHETIC_EV_DATA.csv manually",
            type=["csv"]
        )
